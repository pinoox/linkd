...
pub fn print_welcome_guide() {
    ...
    print_cmd(
        "doctor",
        "[--explain <topic>]",
        "Validate environment, permissions & store health",
    );
    println!();
    ...
    print_cmd(
        "register",
        "[path] (pin, add)",
        "Register current package globally for reuse",
    );
    print_cmd(
        "use",
        "<package> [consumer]",
        "Link a registered package into the current project",
    );
    print_cmd(
        "packages",
        "(pinned)",
    ...
    print_cmd(
        "link",
        "<source> [consumer]",
        "Link a local package directly into a project",
    );
    print_cmd(
        "unlink",
        "<package>",
        "Safely disconnect package link and clean markers",
    );
    print_cmd(
        "list",
        "",
        "List all active synchronized links across projects",
    );
    print_cmd(
        "status",
        "[--json]",
        "Check daemon health, process PID & sync state",
    );
    println!();
    ...
    print_cmd(
        "monitor",
        "(top, dashboard)",
        "Launch real-time interactive full-screen TUI",
    );
    print_cmd(
        "start",
        "",
        "Launch background continuous reconciler daemon",
    );
    print_cmd(
        "stop",
        "[--force]",
    ...
    print_cmd(
        "watch",
        "",
        "Run foreground link watcher with live event logs",
    );
    print_cmd(
        "logs",
        "[-f / --follow]",
        "Inspect or tail daemon event logs",
    );
    println!();
    ...
    print_cmd(
        "version",
        "[-v, --json]",
        "Display version, target OS and ecosystem info",
    );
    print_cmd(
        "completions",
        "<bash|zsh|fish|pwsh>",
    ...
    println!(
        "    {}  {}",
        "$ linkd init".cyan(),
        "# Interactive setup wizard".dark_grey()
    );
    println!(
        "    {}  {}",
        "$ cd packages/ui && linkd register".cyan(),
        "# Register package globally".dark_grey()
    );
    println!(
        "    {}  {}",
        "$ cd apps/web && linkd use @acme/ui".cyan(),
        "# Use registered package in app".dark_grey()
    );
    println!(
        "    {}  {}",
        "$ linkd link ./packages/core ./apps/backend".cyan(),
        "# Direct local link".dark_grey()
    );
    println!(
        "    {}  {}",
        "$ linkd monitor".cyan(),
        "# Open live terminal dashboard".dark_grey()
    );
    println!();
    ...
    println!(
        ...
        "JS/TS (npm/pnpm/yarn/bun) • PHP (Composer) • Python (uv/pip) • Go • Rust • JVM • Custom"
            .dark_grey()
    );
    println!(
        "    {} {}",
